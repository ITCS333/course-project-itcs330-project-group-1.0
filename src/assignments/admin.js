...
async function handleAddAssignment(event) {
  ...
  if (addBtn.dataset.editId) {
    await handleUpdateAssignment(parseInt(addBtn.dataset.editId, 10), { title, due_date, description, files });
    return;
  }
  ...
  if (result.success) {
    assignments.push({ id: result.data.id, title, due_date, description, files });
    renderTable();
...
async function handleUpdateAssignment(id, fields) {
  ...
  if (result.success) {
    ...
    if (index !== -1) {
      assignments[index] = { ...assignments[index], ...fields };
    }
    renderTable();
    assignmentForm.reset();
