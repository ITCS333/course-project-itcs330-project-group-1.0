...
function updateUser($db, $data) {
    ...
    if (isset($data['email']) && $data['email'] !== '') {
        ...
        $dupCheck = $db->prepare('SELECT id FROM users WHERE email = :email AND id != :id');
        $dupCheck->execute([':email' => $data['email'], ':id' => (int)$data['id']]);
        if ($dupCheck->fetch()) {
...
function changePassword($db, $data) {
    ...
    $update->execute([':password' => $newHash, ':id' => (int)$data['id']]);
...
try {
    ...
    error_log('PDOException in admin/api/index.php: ' . $e->getMessage());
    sendResponse('A database error occurred.', 500);

} catch (Exception $e) {
    // TODO: Call sendResponse() with the exception message and HTTP 500.
    error_log('Exception in admin/api/index.php: ' . $e->getMessage());
