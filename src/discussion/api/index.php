...
function updateTopic(PDO $db, array $data): void
{
    ...
    $params[] = (int)$data['id'];
    $stmt = $db->prepare('UPDATE topics SET ' . implode(', ', $setClauses) . ' WHERE id = ?');
    $stmt->execute($params);
...
function createReply(PDO $db, array $data): void
{
    ...
    // If not, sendResponse HTTP 404.
    $check = $db->prepare('SELECT id FROM topics WHERE id = ?');
    $check->execute([(int)$data['topic_id']]);
    if (!$check->fetch()) {
        sendResponse(['success' => false, 'message' => 'Topic not found.'], 404);
    }
    ...
    if ($stmt->rowCount() > 0) {
        ...
        sendResponse(['success' => true, 'message' => 'Reply created.', 'data' => $reply], 201);
    } else {
...
try {
    ...
    error_log('PDOException in discussion/api/index.php: ' . $e->getMessage());
    sendResponse(['success' => false, 'message' => 'A database error occurred.'], 500);

} catch (Exception $e) {
    // TODO: Log the error with error_log().
    // Return HTTP 500 using sendResponse().
    error_log('Exception in discussion/api/index.php: ' . $e->getMessage());
