...
async function initializePage() {
  ...
  const [topicRes, repliesRes] = await Promise.all([
    fetch(`./api/index.php?id=${currentTopicId}`).then(r => r.json()),
    fetch(`./api/index.php?action=replies&topic_id=${currentTopicId}`).then(r => r.json()),
  ]);

  currentReplies = (repliesRes.success && repliesRes.data) ? repliesRes.data : [];

  if (topicRes.success && topicRes.data) {
    renderOriginalPost(topicRes.data);
    renderReplies();
